...
export const RegTimer = ({ deadline, compName }) => {
    ...
    const getTime = (deadline) => {
        ...
        setSeconds(remainingSeconds);
        // Check for the exact 30-minute mark
        if (remainingDays === 0 && remainingHours === 0 && remainingMinutes === 1 && remainingSeconds === 0 && compName === "contest") {
            console.log("horha");
...
export const RunningTimer = ({ deadline, compName }) => {
    ...
    const isValidTime = timeLeft > 0 && timeLeft <= 3 * 60 * 60 * 1000; // 3 hours in milliseconds
